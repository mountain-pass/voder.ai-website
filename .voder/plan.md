# Implementation Plan# Implementation Plan# Implementation Plan# Implementation Plan# Implementation Plan# Implementation Plan# Implementation Plan



Based on the assessment in implementation-progress.md, we have 41 E2E test failures:

- 12 obsolete tests that need removal (Progressive Reveal tests for Act 1 elements)

- 29 legitimate failures that need fixing## NOW



Following Gall's Law, we'll start with the simplest fixes first.



## NOW**Commit security incident documentation for tar vulnerability**## NOW



**Remove obsolete Progressive Reveal tests from scroll-locked-reveal.spec.ts**



The 12 Progressive Reveal test failures are testing obsolete behavior. The implementation in `src/scroll-locked-reveal.ts` explicitly skips Act 1 elements (lines 118-121) because they're handled by MagicPhaseAnimator. However, ALL elements with `data-reveal-start` in the HTML have `data-act="1"`, so these tests are validating behavior that was intentionally refactored away.The assessment identified a tar@7.5.1 vulnerability that has been properly documented as an accepted residual risk per security policy. This documentation needs to be committed to the repository.



**Action**: Delete or update the three failing Progressive Reveal test cases:

1. "should progressively reveal elements as user scrolls" 

2. "should fully reveal elements after their timing range"**Actions**:**Fix Security Vulnerabilities and Update Dependencies**## NOW

3. "should handle multiple elements with different timing"

1. Commit the new security incident file: `SECURITY-INCIDENT-2025-11-06-tar-race-condition-accepted-risk.accepted.md`

These tests fail across all 4 browsers (Chromium, WebKit, Mobile Chrome, Mobile Safari) = 12 total failures.

2. Commit the removal of the incorrectly placed problem file: `npm-override-tar-vulnerability.known-error.md`

**Expected outcome**: Reduce test failures from 41 to 29, improving pass rate from 90.6% to 93.2%.
...
**Fix Layout Integrity issue at 320px viewport**
...
5 test failures (1 per browser + Mobile Safari) for horizontal overflow at 320px breakpoint.



**Decision required**: 

- Option A: Add CSS to support 320px (iPhone SE, older devices)## NEXT

- Option B: Update test to start at 375px minimum instead

### 1. Fix Moderate Security Vulnerability in `tar`**Fix E2E Test Failures (21 failures blocking Phase 6 Runtime Validation)**## NOW

**Action**: Review if 320px support is a requirement, then either:

- Add CSS media query for 320px width to prevent horizontal overflow, OR**Continue Phase 2 of animation system migration (Problem 013)**

- Update functional-layout.test.ts to test starting at 375px instead of 320px



**Expected outcome**: Fix 5 test failures.
...
1. **Fix Screenshot Generation Timeouts** (4 failures)

   - Increase timeout from 30s to 45s or 60s for font loading1. Debug MagicPhaseAnimator test failures:

   - OR preload fonts before screenshot tests

   - Files: `tests/e2e/screenshots.spec.ts`   - Fix Segment 1 transform format and glow effects



2. **Fix Test Execution Timeouts** (3 failures)   - Fix Segment 2 snap-back animation behavior**Actions:**The E2E test suite shows 21 failures across multiple categories that need to be addressed:

   - Debug why tests hang in: narrative-height-validation.spec.ts, mobile-cube-resize.test.ts

   - May need to increase timeouts or fix race conditions2. Complete Phase 2 migration ensuring all 377 tests pass



3. **Fix FOUC Prevention CLS Score** (2 failures - HIGH IMPACT)3. Proceed to Phase 3 (Validation) for end-to-end testing1. Update `netlify-cli` which should bring in the fixed version of `tar`:

   - CLS score is 1.0, needs to be < 0.1 for good Core Web Vitals
...
4. **Investigate Performance Budget Failures** (2 failures)

   - Mobile operations taking 18-20s vs 12s budget

   - Determine if real performance issue or budget needs adjustment

   - Files: `tests/e2e/3d-cube-performance.spec.ts`## LATER   npm update netlify-cli@23.10.0



5. **Fix Scroll Narrative Detection Console Log Capture** (13 failures - COMPLEX)

   - Console logs exist in code but E2E tests can't capture them

   - May be suppressed in production build or timing issue**Optional dependency updates after maturity period**   ```### Priority 1: P003 Button Overlap Issues (6 failures)Fix the test failure in `scroll-locked-reveal.test.ts` that is blocking all development.## NOW## NOW

   - Files: `tests/e2e/scroll-narrative-detector.spec.ts`

   - This is the most complex fix and should be done last



**Note**: After each fix, re-run E2E tests to verify the fix and check for any regressions.Several packages have updates available but are either too fresh (< 7 days) or require breaking change analysis.
