 ...
 */
export function setupJsdomTestEnvironment() {
    const isVitest = typeof globalThis !== 'undefined' &&
        (Boolean(globalThis.__vitest__) || Boolean(process.env.VITEST));
    ...
    try {
        if (typeof HTMLCanvasElement !== 'undefined') {
            ...
            if (verbose) {
                console.error('[voder/ui-tools] Installed Canvas 2D mock to prevent axe-core crashes in JSDOM');
            }
        }
    }
    catch (err) {
