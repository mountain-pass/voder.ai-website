...
- [ ] **Vercel CLI Status Verification**: Use `vercel ls` to list deployments and `vercel inspect <deployment-url>` to verify the deployment is healthy and successful. NOTE: an error in the deployment means this acceptance criteria fails. It only passed if the deployment is successful.
