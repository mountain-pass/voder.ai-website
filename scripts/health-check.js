...
function checkLockfileAndNodeModules() {
  ...
  if (!fs.existsSync(lockPath)) {
    ...
    console.error('Remediation: run `npm install` to generate package-lock.json and install dependencies.');
  ...
  if (!fs.existsSync(nodeModulesPath)) {
    ...
    console.error('Remediation: run `npm ci` to install dependencies (recommended) or `npm install`.');
    ok = false;
  }
  return ok;
}
...
async function main() {
  ...
  for (const step of steps) {
    ...
    if (r.code !== 0) {
      ...
      if (step.label === 'TypeScript type-check') {
        console.error('Remediation: run `npm run type-check` locally and address type errors reported above.');
      } else if (step.label.includes('ESLint')) {
        console.error('Remediation: run `npm run lint:fix` to try to auto-fix issues, then `npm run lint:check` to verify.');
      } else if (step.label.includes('Prettier')) {
        console.error('Remediation: run `npm run format` to auto-format files, then `npm run format:check`.');
      }
...
main().catch((err) => {
  console.error('Health-check encountered an unexpected error:', err && err.stack ? err.stack : err);
  process.exit(3);
});
