...
function setupAdrSymlinks(packageDir, packageName, quietMode = false) {
  ...
  for (const file of fs.readdirSync(adrSourceDir)) {
    ...
    const linkPath = path.join(adrTargetDir, file);
    try { fs.symlinkSync(relativeTarget, linkPath); if (!quietMode) console.log(`    Linked ADR ${file}`); linked++; } catch (e) { console.error(`    Failed to link ADR ${file}: ${e.message}`); }
  }
  if (!quietMode) console.log(`  ADR linking complete (${linked} linked)`);
  const summaryPath = path.join(adrTargetDir, 'README.md');
  try {
    const entries = fs.readdirSync(adrTargetDir).filter(f => f.startsWith('adr-') && f.endsWith('.md'));
    const lines = [
      `# Decision Records for ${packageName}`,
      '',
      'The following ADRs are linked here because they declare applicability to this package via front-matter `packages:` tag.',
      '',
      ...entries.map(e => `- [${e}](./${e})`),
      '',
